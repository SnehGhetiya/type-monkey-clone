              ...
              <Textarea
                ...
                onChange={(e: any) => {
                  ...
                  if (e.inputType !== "deleteContentBackward") {
                    // Ignore backspace
                    const newChar = e.target.value.slice(-1); // Get the newly typed character
                    const charElement = document.querySelector(
                      `.char-${inputParagraph.length - 1}`
                    ); // Target the last character element
                    if (charElement) {
                      charElement.classList.add("animate-type");
                      charElement.classList.remove("animate-type"); // Reset animation after each keystroke
                    }
                  } else {
                    // Handle backspace: target the element before the last character
                    const charElement = document.querySelector(
                      `.char-${inputParagraph.length}`
                    ); // Adjust if needed based on your rendering logic
                    if (charElement) {
                      charElement.classList.remove("animate-type"); // Remove animation on backspace
                    }
                  }
                }}
                className="text-2xl lg:text-5xl outline-none p-5 absolute top-0 left-0 right-0 bottom-0 z-10 opacity-75 cursor-blink"
                placeholder=""
